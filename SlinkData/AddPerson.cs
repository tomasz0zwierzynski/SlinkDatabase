...
namespace SlinkData {
   public partial class AddPerson : Form {
      ...
      private void readOccupations() {
         string commandText = "SELECT `id`, `name`, `description` FROM `occupations`; ";
      ...
      private bool addPersonToDatabase() {
         ...
         string commandText = "INSERT INTO `people` (`id`, `forename`, `surname`, `alias`, `occupation`, `occupation2`, `phonenumber`, `e-mail`, `link`, `comment`) VALUES(NULL, @forename, @surname, @alias, @occupation, @occupation2, @phonenumber, @email, @link, @comment) ";
         using (MySqlConnection connection = new MySqlConnection(context.ConnectionString)) {
            MySqlCommand command = new MySqlCommand(commandText, connection);
            if (forename == "") {
