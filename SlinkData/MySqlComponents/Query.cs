...
namespace SlinkData {
   public static class Query {
      ...
      public const String insertIntoClubs = @"
         INSERT INTO `clubs` (`id`, `name`, `town`, `link`, `rank`, `manager`, `e-mail`,`facebook`,`comment`)
         VALUES(NULL, @name, @town, @link, @rank, @manager, @email, @facebook, @comment) ";
      ...
      public const String updatePub = @"
         UPDATE `clubs`
         SET `name` = @name, `town` = @town, `link` = @link, `rank` = @rank, `manager` = @manager, `e-mail` = @email, `facebook` = @facebook,`comment` = @comment
         WHERE `clubs`.`id` = @id ;";
